...
public class ScoreboardManager {
	...
	public void sendTeams(@Nonnull Player p) {
		...
		for (ScoreboradTeam team :
			...
			if (CoreConfig.getInstance().getBoolean("prefix.overrideSuffixClanTags") && ClanAPI.getInstance()
			                                                                                   .isOnline()) {
				if (NickAPI.getInstance().isNicked(team.getPlayer())) {
					...
					if (clan != null) {
						...
						scoreTeam.setSuffix(CoreConfig.getInstance()
						                              .getString("prefix.clanTagFormat")
						                              .replace("%color%", color)
						                              .replace("%tag%", clan.getTag().complete()));
					} else {
						scoreTeam.setSuffix(team.getSuffix());
					}
				}
