...
public class PrimeCoreCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (args.length == 0) {
            p.thePlayer()
                    .sendMessage("PrimeCore v" + PrimeCore.getInstance().getDescription().getVersion() + " by PrimeAPI");
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "update": {
                ...
                if (args.length < 2) {
                    ...
                    p.thePlayer()
                            .sendMessage("§8[§cSpigotAPI§8] §ePluginName §8| §7Aktuelle Version §8| §bNeueste Version ");
                    for (RestPlugin plugin : PrimeCore.getInstance().getRestManager().getPlugins()) {
                        ...
                        if (info.isNeverVersion(currVersion)) {
                            p.thePlayer()
                                    .sendMessage(
                                            "§8[§cSpigotAPI§8] §e" + plugin.getName() + " §8| §7" + currVersion + "§8 | §b" + info.getLatest() + " §c§l✖");
                        } else {
                            p.thePlayer()
                                    .sendMessage(
                                            "§8[§cSpigotAPI§8] §e" + plugin.getName() + " §8| §7" + currVersion + "§8 | §b" + info.getLatest() + " §a§l✔");
                        }
                    }
                    return true;
                }
                if (args[1].equalsIgnoreCase("all")) {
                    for (RestPlugin plugin : PrimeCore.getInstance().getRestManager().getPlugins()) {
                        ...
                        try {
                            ...
                            if (plugin.downloadLatestVersion(f.getPath())) {
                                p.thePlayer()
                                        .sendMessage(
                                                "§8[§cSpigotAPI§8] §e" + plugin.getName() + "§7 wurde " + "§aerfolgreich §7in" +
                                                        " die Datein §e" + f.getName() + "§7 runtergeladen!");
                            } else {
                                p.thePlayer()
                                        .sendMessage(
                                                "§8[§cSpigotAPI§8] §e" + plugin.getName() + "§7 konnte §cnicht §7herruntergeladen" +
                                                        " werden");
                            }
                    ...
                    PrimeCore.getInstance()
                            ...
                            .getPlugins()
                            .stream()
                            .filter(plugin -> plugin.getName().equalsIgnoreCase(args[1]))
                            .forEach(plugin -> {
                                p.thePlayer()
                                        .sendMessage("§8[§cSpigotAPI§8] §7Installiere §e" + plugin.getName() + "§7...");
                                try {
                                    File f = new File(plugin.getClass()
                                            .getProtectionDomain()
                                            .getCodeSource()
                                            .getLocation()
                                            .toURI());
                                    ...
                                    p.thePlayer()
                                            .sendMessage(
                                                    "§8[§cSpigotAPI§8] §e" + plugin.getName() + "§7 konnte §cnicht " +
                                                            "§7herruntergeladen werden: §c" + e.getMessage());
                                }
                            });
                }
            }
