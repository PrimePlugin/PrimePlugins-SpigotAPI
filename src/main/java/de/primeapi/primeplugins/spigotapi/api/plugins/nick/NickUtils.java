...
public class NickUtils {
	...
	public static void setSkin(Player player, String value, String signature) {
		...
		try {
			Class<?> BUKKIT_PLAYER = Class.forName("org.bukkit.craftbukkit." + PrimeCore.getInstance()
			                                                                            .getVersionManager()
			                                                                            .getNmsVersion() + ".entity" +
					                                       ".CraftPlayer");
			...
			Object craftPlayer = BUKKIT_PLAYER.cast(
					player);                                                            // BUKKIT_PLAYER
			GameProfile gameProfile = (GameProfile) BUKKIT_PLAYER.getMethod("getProfile")
			                                                     .invoke(craftPlayer);   // GameProfile (non nms)
			Object handle = BUKKIT_PLAYER.getMethod("getHandle")
			                             .invoke(craftPlayer);                            // ENTITY_PLAYER
			Object handleArray = Array.newInstance(ENTITY_PLAYER, 1);
			Array.set(handleArray, 0, handle);

			//EntityPlayer handle = (((CraftPlayer) player).getHandle());

			Class<?> PACKET_PLAYOUT_PLAYER_INFO = PrimeCore.getInstance()
			                                               .getVersionManager()
			...
			Constructor<?> PACKET_PLAYOUT_PLAYER_INFO_CONST = PACKET_PLAYOUT_PLAYER_INFO.getConstructor(
					PACKET_PLAYOUT_PLAYER_INFO_ENUM, Class.forName("[L" + ENTITY_PLAYER.getName() + ";"));
			Method PACKET_PLAYOUT_PLAYER_INFO_ENUM_BYSTRING = PACKET_PLAYOUT_PLAYER_INFO_ENUM.getMethod(
					"valueOf", String.class);
			PACKET_PLAYOUT_PLAYER_INFO_CONST.setAccessible(true);
			PACKET_PLAYOUT_PLAYER_INFO_ENUM_BYSTRING.setAccessible(true);


			Class<?> PACKET_PLAYOUT_ENTITY_DESTROY = PrimeCore.getInstance()
			                                                  .getVersionManager()
			                                                  .getNMSClass("PacketPlayOutEntityDestroy");

			Constructor<?> PACKET_PLAYOUT_ENTITY_DESTROY_CONST = PACKET_PLAYOUT_ENTITY_DESTROY.getConstructor(
					Class.forName("[I"));

			Class<?> PACKET_PLAYOUT_ENTITY_SPAWN = PrimeCore.getInstance()
			                                                .getVersionManager()
			                                                .getNMSClass("PacketPlayOutNamedEntitySpawn");
			Constructor<?> PACKET_PLAYOUT_ENTITY_SPAWN_CONST =
					PACKET_PLAYOUT_ENTITY_SPAWN.getConstructor(HUMAN_PLAYER);
